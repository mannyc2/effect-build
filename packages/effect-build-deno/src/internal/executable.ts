...
export const inspectNativeExecutable = (bytes: Uint8Array): NativeObservation => {
...
const targetDescriptor = (target: SupportedTarget): NativeObservation => {
  switch (target) {
    ...
    case "windows-aarch64":
      return { nativeFormat: "pe", os: "windows", architecture: "aarch64" };
  }
};
...
export const matchesNativeTarget = (target: SupportedTarget, observation: NativeObservation): boolean =>
...
const withExeSuffix = (basename: string, target: SupportedTarget | undefined): string =>
  (target === "windows-x64" || target === "windows-aarch64") && !basename.toLowerCase().endsWith(".exe")
    ? basename + ".exe"
    : basename;
...
export const publishExecutable = <Mode extends ObservationMode, ProviderFailure>(
  input: {
    ...
    /**
     * Runs before resolving the destination or allocating a private candidate.
     * Providers use this for inputs that their native compiler would otherwise
     * lazily acquire after staging has started.
     */
  ...
  Effect.scoped(
    Effect.gen(function*() {
      ...
      const artifact = {
        ...
        runtime: { name: "deno", version: input.runtimeVersion },
        target,
