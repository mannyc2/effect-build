...
import type {
  ...
  DecimalBytes,
  Digest,
  HashedFileObservation as CanonicalHashedFileObservation,
  HashedTreeEntry,
  HashedTreeObservation as CanonicalHashedTreeObservation,
  ObservationMode,
  PortableRelativePath,
  TreeDirectoryEntry as CanonicalTreeDirectoryEntry,
  TreeObservation as CanonicalTreeObservation,
  UnhashedFileObservation as CanonicalUnhashedFileObservation,
  UnhashedTreeEntry,
  UnhashedTreeObservation as CanonicalUnhashedTreeObservation,
} from "../Artifact.js";
...
export type UnhashedFileObservation = CanonicalUnhashedFileObservation;
export type HashedFileObservation = CanonicalHashedFileObservation;
export type TreeDirectoryEntry = CanonicalTreeDirectoryEntry;
export type { HashedTreeEntry, UnhashedTreeEntry, UnhashedTreeFileEntry } from "../Artifact.js";
export type UnhashedTreeObservation = CanonicalUnhashedTreeObservation;
export type HashedTreeObservation = CanonicalHashedTreeObservation;
...
export type TreeObservation<Mode extends ObservationMode> = CanonicalTreeObservation<Mode>;
...
interface PrivateTreeObservation {
  readonly root: AbsolutePath;
  readonly rootMode: ReturnType<typeof fileMode>;
...
const portablePath = (entry: string): PortableRelativePath | undefined => {
  if (
    entry.length === 0
...
const observePrivateTree = (
  ...
  Effect.gen(function*() {
    ...
    while (pendingDirectories.length > 0) {
      ...
      const absoluteDirectory = directory.length === 0 ? root : path.join(root, directory);
      const children = yield* fileSystem.readDirectory(absoluteDirectory).pipe(
        Effect.mapError((error) => new BorrowedOutputObservationFailed({ leaseId, reason: describe(error) })),
      );
      for (const child of children.sort(comparePath)) {
        const name = directory.length === 0 ? child : path.join(directory, child);
        names.push(name);
        const absolute = path.join(root, name);
        if (Option.isSome(yield* Effect.option(fileSystem.readLink(absolute)))) continue;
        const information = yield* fileSystem.stat(absolute).pipe(
          Effect.mapError(() => new BorrowedOutputMissing({ leaseId, path: absolute })),
        );
        if (information.type === "Directory") pendingDirectories.push(name);
      }
    }
    ...
    for (const name of names) {
      ...
      if (Option.isSome(link)) {
        ...
        if (Option.isNone(yield* Effect.option(fileSystem.stat(resolved)))) {
          return yield* new BorrowedOutputObservationFailed({
            leaseId,
            reason: `tree contains an absent, broken, or cyclic symbolic link: ${relativePath}`,
          });
        }
      ...
      if (information.type === "Directory") {
        entries.push(Object.freeze({
          ...
          relativePath,
          mode: fileMode(Number(information.mode) & 0o7777),
        }));
        continue;
      }
      if (information.type !== "File") {
    ...
    const manifestDigest = yield* digest(
      leaseId,
      encoder.encode(JSON.stringify({ rootMode, totalBytes, entries: frozen })),
    );
    return Object.freeze({ root, rootMode, entries: frozen, totalBytes, manifestDigest });
  });
...
const publicTree = <Mode extends ObservationMode>(
  ...
  const entries: UnhashedTreeEntry[] = observed.entries.map((entry) =>
    entry.kind === "directory" || entry.kind === "symbolic-link"
      ? entry
      : Object.freeze({
        kind: "file" as const,
