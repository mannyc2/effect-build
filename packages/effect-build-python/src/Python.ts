...
export const build = Effect.fn("Python.build")((input: BuildInput): Effect.Effect<BuildResult, BuildError, Python | Env> =>
  Effect.gen(function*() {
    for (const [field, value] of [["project", input.project], ["outdir", input.outdir]] as const) {
      ...
      if (issue !== undefined) return yield* new InputInvalid({ reason: `${field} ${issue}` });
    }
    ...
    const produce = (out: string) => Effect.gen(function*() {
      ...
      if (directory.entries.filter(topLevel(".whl")).length !== 1 || directory.entries.filter(topLevel(".tar.gz")).length !== 1) {
