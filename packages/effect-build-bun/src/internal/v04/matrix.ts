...
): Effect.Effect<CoreMatrix.Report<Artifact, Error, "bun">, CoreMatrix.InvalidInput> =>
  Effect.gen(function*() {
    ...
    const cells = yield* Effect.forEach(
      ...
      (cell, index) =>
        capture(scalar(cell)).pipe(
          Effect.map((result): CoreMatrix.CellResult<Artifact, Error, "bun"> => {
            const identity = { provider: "bun" as const, operation: "compileExecutable" as const, index };
    ...
    return { provider: "bun", operation: "compileExecutable", cells, rollback: "none" };
  });
