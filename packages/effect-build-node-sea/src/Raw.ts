...
import type { ArtifactInvalid, SelectedToolChanged, ToolNotFound } from "effect-build/BuildError";
...
export interface AssembleExecutableInput {
  ...
  /** Required; the selected base must be independently proved for this target. */
  readonly target: CoreTarget.Target;
  readonly cwd?: string;
...
> =>
  Effect.gen(function*() {
    ...
    const produce = (
      ...
      void,
      ToolFailed | PublishFailed | ArtifactInvalid | SelectedToolChanged,
      Crypto.Crypto | FileSystem.FileSystem | Path.Path | ChildProcessSpawner.ChildProcessSpawner
    > =>
      Effect.gen(function*() {
        ...
        const check = yield* Toolchain.runSelected({ selected: tool, args: ["--check", main], cwd });
        if (check.exitCode !== 0) {
        ...
        yield* Effect.asVoid(
          Toolchain.runOrFailSelected({ selected: tool, args: ["--build-sea", configPath], cwd }),
        );
      });
