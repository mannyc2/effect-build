...
format="$1"
package_path="$2"
receipt_path="${package_path}.native.sha256"
...
if [[ ! -f "$receipt_path" ]]; then
  echo "native compiler payload digest receipt is missing" >&2
  exit 64
fi
...
"$docker_bin" run --rm --network none --mount "type=bind,src=$package_path,dst=$container_package,readonly" --mount "type=bind,src=$receipt_path,dst=/acceptance/native.sha256,readonly" "$image" sh -c "$oracle"
