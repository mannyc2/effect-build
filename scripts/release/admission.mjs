...
export const selectAdmission = ({
  ...
  const packageCount = releaseControl.orderedPackages.length;
  const allAbsent = registry.length === packageCount && all(registry, "Absent");
  const allEquivalent = registry.length === packageCount && all(registry, "Equivalent");
  if (githubCoordinatorState === "TagAbsent") {
    return !expired && actionsArtifacts === "Available" && allAbsent && origin === "initial-main-attempt-one"
  ...
  if (["TagEquivalentReleaseAbsent", "DraftEquivalentEscrowAbsent"].includes(githubCoordinatorState)) {
    return actionsArtifacts === "Available" && allAbsent && origin === "same-attempt-continuous-lease"
  ...
  if (githubCoordinatorState === "DraftEquivalentEscrowAndFinalAssetsComplete") {
    if (allEquivalent) return { _tag: "Admit", arm: "escrow-present-github-finalization" };
    if (prefix.length < packageCount && ["Available", "ExpiredOrDeleted"].includes(actionsArtifacts)) {
