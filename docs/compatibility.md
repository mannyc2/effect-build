...
| esbuild / Rolldown                               | Their pinned npm native packages on supported hosts               | Bundles and scoped native APIs. Portable unit tests and installed imports run on three OSes.                                                                                                            |
...
| Windows signing                                  | Windows SDK SignTool, Windows host                                | Native CI signs, timestamps, verifies and runs a PE executable with a temporary self-signed certificate. Production certificates and native MSIX signing remain experimental and unverified.            |
| Apple products / signing / notarization          | macOS, Xcode command-line tools, appropriate credentials          | Native unsigned app construction checked locally; credentialed distribution remains experimental and covered by scripted processes only. Universal/fat Mach-O inputs are unsupported.                   |
