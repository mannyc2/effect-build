...
Release authority is governed by `effect-build/research-complete-contract@1` in
[`tooling/research-complete-contract.json`](../tooling/research-complete-contract.json).
The v0.5 snapshot still describes the existing quarantined coordinator. The unsafe
...
The candidate job uploads the six-tarball payload first and then a detached
canonical descriptor. The payload is exactly six top-level regular `.tgz` files
...
distribution coordinates, and the exact eight clean-host coordinates. Before
that opaque bundle is built, the certification workflow strictly validates
canonical `apple-certification-request@2`, `receipt@2`, `evidence@2`, and
`bundle@2` records. Receipts bind the request digest, exact package/source,
`bun.lock`, clean worktree, runner, configured certifier path, and a protected
approved certifier digest. Distribution/cell and clean-host digests are distinct;
only an authenticated read-only private snapshot is executed. Verifier-owned
prior-evidence manifests bind an exact, privately snapshotted dependency set;
clean-host transports are identity-equal to their matching distribution output,
and aggregation revalidates every manifest/file/artifact cross-link. Category-specific evidence
must enumerate artifacts, tools, and required successful operations, with
credential/Notary proof for distribution, prior/quarantine/user-flow proof for
clean-host cells, and cell-specific claims for A0–A9. The release coordinator
authenticates the enclosing bytes but never decodes or freezes A7's provisional
Notary provider JSON/status or detailed receipt/reconciliation shapes.

The repository cannot prove the deployed `apple-certification` environment
variables, reviewer rules, same-UID isolation, approved interpreter/toolchain
identities, runner-resident certifier bytes, credentials, or clean-host isolation.
Protected-environment readback and all 32 exact-candidate
credentialed receipts remain external release gates.
...
draft must contain and verify that temporary escrow plus all eight final assets—nine
...
Once that escrow and all eight final assets are complete, logical expiry or
...
certification artifact subject, opaque certification bundle, six tarballs,
...
This wrapper-recovery rule applies only while npm work remains. Once all six
...
`Unknown`; `NotReached` is report-only. Preflight all six before mutation. An
...
temporary escrow container, and then uploads the six candidate tarballs, one
...
eight final assets. The manifest embeds the exact descriptor, canonical
...
Only after all six npm coordinates converge may the coordinator delete the
temporary escrow asset, verify the exact eight final assets, and publish the
non-prerelease draft as latest. A lost response after escrow deletion resumes
from the exact phase-correct tag and draft, eight final assets, canonical manifest
and its embedded candidate/certification/escrow subjects, and six authoritative
...
the six-package candidate, but no Apple product-form release authority is
earned by that source integration. The generated research-complete contract
accounts all nine modules as implemented and records focused non-credential and
...
checks without a retained exact-revision receipt or CI record; those results do
not transfer. A0–A9, all 14 credential-backed distribution coordinates, and all
eight G product-form × architecture clean-host coordinates remain formally
open. Promotion of either `effect-build-apple` or v0.5 stops until the complete
exact-candidate evidence is retained.
...
governed by the one protected `npm` environment and fixed-six convergence
protocol below.
...
exactly `mannyc2/effect-build`; all six package bindings, npm trusted-publishing
...
`effect-build-apple` was anonymously observed absent from npm on 2026-08-23. It
needs a separately authorized, audited reservation publication before its
trusted-publisher binding can be created and read back; staged publishing cannot
bootstrap an absent coordinate. `effect-build-rolldown` is instead a private,
conditional research candidate: it is not part of the six-package descriptor,
npm convergence, trusted-publisher bindings, or release assets. This repository
change does not authorize or perform namespace bootstrap, trusted-publisher
changes, npm publication, Git tags, or a GitHub Release. Those remain separate
explicit actions. Successful publication must later be followed by anonymous
tarball, digest, provenance, dist-tag, consumer, manifest, tag, and Release-asset
verification.
