...
| esbuild          | peer `>=0.28.2 <0.29.0`      | Tested with 0.28.2; the consumer's own esbuild runs in process, no tool layer.                                                                                              |
| Rolldown         | npm dependency `1.2.5`       | In-process API; no tool layer. Pinned: the wrapper uses `rolldown/experimental`, whose types move outside semver.                                                           |
