...
- effect-build turns an Effect application into deployable artifacts — native executables and bundles via Bun, Deno, esbuild, Node SEA, and Rolldown — as composable Effect programs. Build capability users actually want; prefer shipping a reviewed increment over deferring it.
- Six lockstep packages: `effect-build` (core) owns `Target`, `Artifact`, `BuildError`, and the `Toolchain` kernel; each provider depends one way on core and never on a provider sibling.
...
- Publication is stage in a private same-parent temp dir → produce → commit: executables get a 4-byte native-magic sanity check and one atomic rename; bundles commit each staged file with a per-file rename, exactly as the native tools overwrite. No double observation, no re-hashing of tools, no TOCTOU defenses.
