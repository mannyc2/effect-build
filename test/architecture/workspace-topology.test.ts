...
const publicPackages = ["effect-build", "effect-build-bun", "effect-build-deno"] as const;
...
describe("Bun workspace package graph", () => {
  it("has one private, exactly pinned workspace root", async () => {
    ...
    expect(manifest.workspaces).toEqual(["packages/*", "examples/bun", "examples/deno"]);
  ...
  it("contains exactly the three lockstep public packages", async () => {
  ...
  it("keeps provider dependencies one-way and ordinary", async () => {
    ...
    for (const [provider, sibling] of [[bun, "effect-build-deno"], [deno, "effect-build-bun"]] as const) {
      expect(provider.dependencies).toEqual({ "effect-build": "workspace:^" });
