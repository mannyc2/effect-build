...
const fixture = () => {
  ...
  const releaseAssets = [...releaseAssetBytes].map(([name, assetBytes], index) => ({
    ...
    browserDownloadUrl: `https://github.com/mannyc2/effect-build/releases/download/v0.6.3/${name}`,
  }));
  ...
  const provenance = names.map((name: string) => {
    ...
    const statement = {
      ...
      subject: [{ digest: { sha512 }, name: `pkg:npm/${name}@0.6.3` }],
      predicate: {
        buildDefinition: {
    ...
    return {
      name,
      attestationUrl: `https://registry.npmjs.org/-/npm/v1/attestations/${encodeURIComponent(name)}@0.6.3`,
...
describe("directly active final-public verifier", () => {
  ...
  it("fails closed on bytes, provenance, smoke, reservation, or Release immutability drift", async () => {
    const cases = [
      ...
      (input: any) => input.reservation.versions.push("0.6.3"),
