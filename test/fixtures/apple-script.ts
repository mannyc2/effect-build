...
import type { TestSpawner } from "effect-build/testing";
...
/** Apple-specific file mutations; transport and interruption belong to TestSpawner. */
export const script = (configuration: () => Config, invocations: Invocation[]): TestSpawner.Script<never> => (call) =>
  Effect.suspend<TestSpawner.Reply, never, never>(() => {
    const config = configuration();
    const [name, ...args] = call.args;
    if (name === "--version" && args.length === 0) return Effect.succeed({ stdout: "xcrun version 70.\n" });
    ...
    if (name === "notarytool" && args[0] === "wait" && config.waitForAbort) return Effect.never;
    return Effect.sync(() => {
      let stdout = "";
      ...
      try {
        const fail = (phase: string) => {
          if (config.fail === phase) {
            throw new ToolFailure(phase + " failed private-notary:$42");
          }
        };
        ...
        const verifySignature = (target: string) => {
          if (lstatSync(target).isDirectory()) {
            if (!existsSync(signature(target))) throw new Error("missing app signature");
          } else if (!readFileSync(target, "utf8").includes(":signed")) throw new Error("missing file signature");
        };
        const target = args.at(-1)!;
        switch (name) {
          case "plutil":
            if (args[0] !== "-lint" || !readFileSync(target, "utf8").includes("<plist")) {
          ...
          case "hdiutil":
            ...
            break;
          case "pkgbuild":
            if (args[0] !== "--component" && args[0] !== "--root") {
              throw new Error("missing installer component or root");
            }
            write(
              target,
              JSON.stringify({
          ...
          case "notarytool": {
            ...
            if (args[0] === "submit" && args.includes("--wait")) {
              throw new Error("submission must return before waiting");
            }
            ...
            emit(
              config.rawResponse
                ?? JSON.stringify(
                  config[operation === "log" ? "logResponse" : operation]
                    ?? (args[0] === "wait" ? config.submit : undefined)
                    ?? defaults,
                ),
            );
            break;
          ...
          case "stapler":
            if (args[0] === "staple") {
              if (lstatSync(target).isDirectory()) {
                write(join(target, "Contents/_CodeSignature/NotaryTicket"), "ticket");
        ...
        return { exitCode: error instanceof ToolFailure ? 37 : 1, stderr: String(error) };
      }
    });
  });
