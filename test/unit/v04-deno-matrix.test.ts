...
import { runMatrix } from "../../packages/effect-build-deno/src/internal/v04/matrix.js";
import * as CoreMatrix from "../../packages/effect-build/src/Matrix.js";

describe("staged 0.4 Deno matrix laws", () => {
  ...
  it("invokes scalar exactly once per cell, bounds concurrency, and reports in input order", async () => {
    ...
    expect(report).toMatchObject({ provider: "deno", operation: "compileExecutable", rollback: "none" });
  ...
  it("captures typed failures as cells, keeps independent commits, and re-fails defects", async () => {
    ...
    expect(report.cells[1]).toEqual({
      _tag: "Failure",
      identity: { provider: "deno", operation: "compileExecutable", index: 1 },
      error: failure,
    });
