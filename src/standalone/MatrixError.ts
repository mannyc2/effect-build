...
import { type BunTarget, bunTargetTable } from "./internal/BunTarget.js";
import { type DenoTarget, denoTargetTable } from "./internal/DenoTarget.js";
...
type DenoArtifact = Artifact.Artifact & {
  readonly target: DenoTarget;
  readonly tool: Artifact.Tool & { readonly name: "deno" };
};

const BunArtifact = Artifact.Artifact.pipe(
  Schema.refine(
    (artifact): artifact is BunArtifact => artifact.tool.name === "bun" && bunTargetTable.isTarget(artifact.target),
    { message: "artifact target must be supported by Bun" },
  ),
);
const DenoArtifact = Artifact.Artifact.pipe(
  Schema.refine(
    (artifact): artifact is DenoArtifact => artifact.tool.name === "deno" && denoTargetTable.isTarget(artifact.target),
