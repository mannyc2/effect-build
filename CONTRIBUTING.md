...
Release rules live in `scripts/effect-build-contract/policy.mjs`. Run `bun run generate:contract` after changing them or a fake-registry fixture; the generator reads fixture bytes and records their digests automatically. `check:contract` checks generated bytes, policy relationships, external evidence, and public implementation coordinates in one pass. Tests exercise release behavior and rejected inputs instead of copying the policy into a second literal snapshot.
