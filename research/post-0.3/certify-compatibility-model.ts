...
const basePolicy = (lane: "api" | "command"): OperationCompatibilityPolicy => ({
  ...
  supportedByProviderPolicy: semVerRange(">=0.28.0 <0.29.0", (version) => version >= "0.28.0" && version < "0.29.0"),
...
conclusion(
  supported._tag === "Compatible" && supported.value._tag === "ProviderPolicySupported",
  "provider-policy admission failed",
);
conclusion(
  override._tag === "Compatible" && override.value._tag === "UntestedOverride",
  "untested override admission failed",
);
conclusion(
  known._tag === "Incompatible" && known.error._tag === "KnownIncompatibility" && !("overrideAvailable" in known.error),
  "known incompatibility advertised an override",
);
conclusion(
  missing._tag === "Incompatible" && missing.error._tag === "MissingCapability"
    && !("overrideAvailable" in missing.error),
  "missing capability advertised an override",
);
conclusion(
  relation._tag === "Incompatible" && relation.error._tag === "RelationUnsatisfied"
    && !("overrideAvailable" in relation.error),
  "relational failure advertised an override",
);
