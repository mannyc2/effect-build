...
if (mode === "success") {
  ...
  console.log(`EFFECT_BUILD_DENO_HOST_RECEIPT=${
    JSON.stringify({
      ...
      memory: {
        ...
        outputs: memory.outputFiles?.map((file) => ({
          path: file.path,
          hash: file.hash,
      ...
      written: {
        ...
        outputs: written.outputFiles?.map((file) => ({
          path: file.path,
          hash: file.hash,
          bytes: file.contents?.byteLength,
        })),
  ...
  try {
    ...
    console.log(`EFFECT_BUILD_DENO_PERMISSION_RECEIPT=${
      JSON.stringify({
