...
scope. Plans 039, 040, and 041 are complete at their export-inert stages: the
six frozen core modules, the two admitted Esbuild operation modules, and the
Bun selected-command executable lane are implemented internally, while the
released 0.3 export map remains byte-identical. Plan 042 is the next
implementation plan. No merge, publication, tag, or release is authorized by
Plan 039, Plan 040, or Plan 041 completion.
...
- Plan 031 makes both native inspectors total over malformed bytes, rejects
  duplicate ELF interpreters and FAT64 deterministically, bounds FAT32 slice
